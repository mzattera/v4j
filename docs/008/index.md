...
```
<BEGIN>:
	 -> 0_d, 0_q, 0_s, 1_o, 1_y, 2_l, 2_r, 3_t|3_p|3_k|3_f, 4_C, 4_S, 5_T|5_P|5_K, 7_d, 8_a
...
As discussed in [Note 006](../006), several models for the inner structure of Voynich terms have been created. The purpose of these models is to provide a description
of such structure in a way that provide some insights (e.g. show that a particular sequence of character is often used). In this process, it is necessary
for any model to operate some choices and simplification in order to highlight what looks like a common pattern disregarding what seems more a "special case".
How can we evaluate how good a model is? In other terms, how well does it describes Voynich terms, without indulging too much in details?
...
|  											| Term T is generated by the model 	| Term T is NOT generated by the model 	|
| :--- 	| :---: 	| :---: 	|
| Term T appears in the Voynich 			| True Positive (TP)				| False Negative (FN) 					|
| Term T does NOT appear in the Voynich 	| False Positive (FP)				| True Negative (TN) 					|
...
We can define the **precision** of a model as the percentage of positives that are true positives; that is, the percentage of terms being generated by the model which do appear in the manuscript.
From the above table we can easily see that:

```
Precision = True Positives / All terms generated = TP / (TP + FP)
```

A model with precision = 1 (or 100%) will generate only words that appear in the Voynich; notice this does not mean it generates ALL of the Voynich terms.
Ideally a model that generates only the term 'daiin' has 100% precision, as all the terms it generates are true Voynich terms, but it ignores all of the other terms.

We can then define the **recall** of a model as the percentage of terms in the Voynich that are generated by the model. Again, from the above table we can easily see that:

```
Recall = True Positives / All terms in the Voynich = TP / (TP + FN)
```

A model with recall = 1 (or 100%) will generate all terms in the text, but it can generate many more that are not in the text.
Potentially, a model that generate all possible combination of letters in the Voynich alphabet will generate all of the terms in the manuscript (recall=1), but will also generate a 
huge amounts of words that are not in the Voynich (false positives).

You can see there is a tension between precision and recall; one can increase recall by having the model to generate more words, but this increases the risk of generating terms which are not Voynich terms 8false positives), thus reducing precision. There is a need of a single number that represent a compromise between best recall and best precision; in the 
theory of classifiers this number if call **F1** or **F-Score** and it is defended as:

```
F1 = ....
```

The table below compares our grammar with other models described in [Note 006](../006), providing their precision, accuracy and F-Score.
