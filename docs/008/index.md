...
I wrote some code [{1}](#Note1) that, starting from the list of terms in the concordance version of the Voynich (see [Note 001](../001)), creates a state machine to model them. The best version I found is described by the below grammar [{2}](#Note2).
...
Potentially, a model that generates all possible combinations of letters in the Voynich alphabet will generate all of the terms in the manuscript (recall=1), but will also generate a 
...
# Conclusions
  ...
  If we ignore these, Neal's model (NEAL_1b) has a higher recall than the proposed one, but generates more than 170'000 words, compared to the only 3'000 thousand generated by my model. 
...
<a id="Note2">**{2}**</a> A parameter `BuildSlotStateMachine` requires is the minimum amount of terms using an edge of the state machine to keep the edge alive (that is, the minimum number of terms containing a specific sequence of two characters). For the machine in this note, I used a minimum weight of 10.
