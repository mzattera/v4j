...
In this note, I create and analyze a graph showing connections between characters in Voynich word types accordingly to my [slot model](../005) [{1}](#Note1);
...
After that, I connected node A with node B if there is a regular word type in the Voynich where character B follows directly character A;
connections are therefore represented a directed edges with a weight equal to the number of regular word types where the characters are connected.
...
This analysis shows that there is a dependency between one character and those preceding it. In other word types,
...
from the [glyph count by slot](../005/#GliphCountImg), some glyphs appear in less than 1% of the word types, that means they will
